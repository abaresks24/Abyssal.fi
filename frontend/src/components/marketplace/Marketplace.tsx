...
function fmtExpiry(d: Date) {
  ...
  const days = Math.floor(diff / DAY); const hrs = Math.floor((diff % DAY) / 3_600_000);
...
function CreateListingForm({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  // Estimated collateral for written options
  const estCollateral = useMemo(() => {
    const s = parseFloat(strike); const sz = parseFloat(size);
    if (!s || !sz) return 0;
    return side === 'put' ? s * sz : s * sz * 2; // 2x for calls
  }, [strike, size, side]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Refresh oracle before writing (needed for collateral calc on-chain)
      if (tab === 'write') {
      ...
      if (tab === 'resale') {
        sig = await client.listForResale({
          ...
          strikeUsdc: s, expiry: expiryTs, sizeUnderlying: sz, askPriceUsdc: a,
        });
      } else {
  ...
  return (
    <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 8, padding: 20, maxWidth: 500 }}>
      ...
      {tab === 'resale' && (
        <p style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 12, lineHeight: 1.5 }}>
          Resell an existing position. You must own a position for this series. The NFT transfers to the buyer.
        </p>
      )}
      {tab === 'write' && (
        <p style={{ fontSize: 11, color: 'var(--amber)', marginBottom: 12, lineHeight: 1.5 }}>
          Write a custom option. You lock collateral in escrow. If someone buys, they pay you the ask price. At expiry, payoff is deducted from your collateral.
        </p>
      )}
      ...
      {/* Collateral preview for written options */}
      {tab === 'write' && estCollateral > 0 && (
        <div style={{ background: 'rgba(236,202,90,0.06)', border: '1px solid rgba(236,202,90,0.15)', borderRadius: 6, padding: '8px 12px', marginBottom: 12 }}>
          ...
          <div style={{ fontSize: 13, fontFamily: 'var(--mono)', color: 'var(--amber)', fontWeight: 600 }}>
            ${fmt(estCollateral)} USDP
          </div>
          <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 2 }}>
            {side === 'put' ? 'strike × size' : '2 × spot × size (on-chain calc)'}
          </div>
        </div>
      )}
      ...
      <button onClick={handleSubmit} disabled={loading || !ready} style={{
        ...
        background: ready ? (tab === 'resale' ? 'var(--cyan)' : 'var(--amber)') : 'var(--bg3)',
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--bg)' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px' }}>
        ...
        {loading ? (
          ...
          <div style={{ padding: '60px 0', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ color: 'var(--text3)', fontSize: 12, maxWidth: 340, lineHeight: 1.6 }}>
              Resell positions or write custom options for other traders.
            </div>
          </div>
        ) : (
              ...
              {filtered.map(l => {
                ...
                return (
                  <tr key={l.pubkey} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ ...td, fontFamily: 'var(--mono)', fontSize: 11, color: l.collateralLocked > 0 ? 'var(--amber)' : 'var(--text3)' }}>
                      {l.collateralLocked > 0 ? `$${fmt(l.collateralLocked, 0)}` : '—'}
                    </td>
        ...
        <div style={{ margin: '24px 0 8px', padding: 14, background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 12, color: 'var(--text3)', lineHeight: 1.6 }}>
          <span style={{ color: 'var(--cyan)', fontWeight: 600 }}>Resale</span> — Resell your existing positions (NFT transfers to buyer). <br />
          <span style={{ color: 'var(--amber)', fontWeight: 600 }}>Written</span> — Write custom options with locked collateral. Buyer pays your ask price. At expiry, payoff deducted from collateral.
        </div>
      </div>
    </div>
  );
